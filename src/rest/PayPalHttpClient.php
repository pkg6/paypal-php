...
trait PayPalHttpClient
{
    ...
    private function doPayPalRequest(bool $decode = true)
    {
        try {
            ...
            return $this->responseDecode(($decode === false) ? $response->getContents() : Utils::jsonDecode($response, true));
    ...
    protected function responseDecode($response)
    {
        if ( ! is_null($this->logger)) {
            $this->logger->write($this->apiUrl, $this->verb, $this->options, $response);
        }
